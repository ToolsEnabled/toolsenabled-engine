...
const { TASK_DIFFICULTY_SETTING_ID, newTaskDifficultyFields, planTaskDifficultyReview } = require('./task-difficulty');
...
function readTransactionHistory(historyFile, document, options, forceVerification = false, adopting = false) {
  ...
  let chain;
  if (forceVerification || adopting || policy.verifyHistory || !policy.configurationAvailable) {
    const stamp = historyStamp(historyFile);
...
function transact(options, now, mutate, forceVerification = false, adopting = false) {
  const { ledgerFile, historyFile } = filesFor(options);
  const clock = clockOf(now);
  const lock = acquireLock(ledgerFile);
  try {
    const document = readDocument(ledgerFile);
    const chain = readTransactionHistory(historyFile, document, options, forceVerification, adopting);
